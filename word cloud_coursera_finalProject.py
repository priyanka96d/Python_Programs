...
def calculate_frequencies(file_contents):
    ...
    list_input = file_contents.split()
    for text in list_input:
        if ((text in punctuations) or (text in uninteresting_words)):
            pass
        else:
            count = sum(1 for _ in re.finditer(r'\b%s\b' % re.escape(text), file_contents))
